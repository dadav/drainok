...
func TestFilterRemovesMachineConfigCheck(t *testing.T) {
	filtered, err := Filter(All(), []string{"machine-config"})
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	for _, check := range filtered {
		if check.Name() == "machine-config" {
			t.Errorf("check %q should have been filtered out", check.Name())
		}
	}
	if len(filtered) != len(All())-1 {
		t.Errorf("expected %d checks, got %d", len(All())-1, len(filtered))
	}
}
